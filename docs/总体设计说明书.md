...
切分输出文档类型和扩展名继承当前文档。`.md/.txt` 来源先按自身编码解码到内部 UTF-8；保存时统一按当前会话编码和换行输出，因此来源与目标编码不需要相同。DOCX/PDF 输入属于 8.3B 条件扩展，使用独立只读 reader 抽取可见文字；扫描 PDF 不引入 OCR，样机门禁未通过时不进入正式功能。
